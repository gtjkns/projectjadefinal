<?php 
	...
	function find_city_by_country($id){
		
		global $connection;
		//$query = "SELECT pdhp_city.name FROM pdhp_city where region_id = " . $country;
		$query = "SELECT id, name FROM pdhp_city where region_id = $id ORDER BY name";
		$city = mysqli_query($connection, $query);
		confirm_query($city);
...
function get_user($id, $table_name)
	{
		global $connection;
		$result = mysqli_query($connection,"select * from $table_name where id = $id");
